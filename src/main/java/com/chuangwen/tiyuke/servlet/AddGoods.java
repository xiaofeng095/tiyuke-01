...
public class AddGoods extends HttpServlet {
		...
		// 中文处理
	        ...
	        try {
	            ...
	            if (formItems != null && formItems.size() > 0) {	
	                ...
	                Dao dao = new Dao();
	                boolean flag = dao.testInser(map.get("id"));
	                if(flag) {
	                int i  = dao.insertImg(map.get("id"), url, map.get("name"), map.get("price"), map.get("num"), map.get("other"));
	                if(i>0) {
	...
	//	response.getWriter().write(upload.parseRequest(request));
	}
